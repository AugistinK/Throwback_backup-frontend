...
const PlaylistSelectionModal = ({ podcastId, onClose, onSuccess }) => {
  ...
  const handleCreatePlaylist = async (e) => {
    ...
    try {
      setSubmitting(true);
      setError(null);
      const response = await podcastAPI.createPlaylist({
      ...
      if (response && response.success) {
        ...
        setNewPlaylistName(''); setNewPlaylistDescription(''); setCreateMode(false);
        setTimeout(() => { if (onSuccess) onSuccess(); }, 1500);
      } else {
        ...
        {!createMode ? (
          <>
            {loading ? (
              ...
              <div className={styles.playlistsContainer}>
                ...
                {isAuthenticated && (
                  <button className={styles.createButton} onClick={() => setCreateMode(true)}>
          ...
          <form onSubmit={handleCreatePlaylist} className={styles.createForm}>
            ...
            <div className={styles.formActions}>
              <button type="button" className={styles.cancelButton} onClick={() => setCreateMode(false)}>
                Cancel
              </button>
              <button type="submit" className={styles.submitButton} disabled={submitting || !newPlaylistName.trim()}>
                {submitting ? <FontAwesomeIcon icon={faSpinner} spin /> : <><FontAwesomeIcon icon={faPlus} /> Create and add</>}
              </button>
            </div>
          </form>
        )}
